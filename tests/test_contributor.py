...
Since Ollama is self-hosted, cost is always indeterminate (None).
Verifies that mount() registers:
  - an `llm:response` hook (_accumulate) that ignores None cost events
  - a lazy contributor callback on session.cost channel under name 'provider-ollama'
...
async def test_contributor_registered_at_mount(mock_coordinator):
    """mount() must register a contributor on ('session.cost', 'provider-ollama')."""
